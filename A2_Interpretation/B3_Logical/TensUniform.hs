...
--
--   Mirrors Tensor.hs (TwoMonBLat + A2MonBLat) exactly, but optimized for:
...
module A2_Interpretation.B3_Logical.TensUniform
  ( BatchOmega,
...
import A2_Interpretation.B3_Logical.Tensor
...
--  (mirrors TwoMonBLat Omega in Tensor.hs)
-- ============================================================
...
veeU a b =
  let ap = powPU (pi0U a)
      bp = powPU (pi0U b)
      meanP = (ap `Torch.add` bp) `Torch.mul` halfU
   in powInvPU meanP
...
-- ============================================================
--  A2MonBLat-U: quantifiers (finite uniform measure)
--  (mirrors A2MonBLat TENS Omega in Tensor.hs)
-- ============================================================
...
bigWedgeU g phi =
  let e  = pi1U phi                    -- π₁ stability
      e' = oneU `Torch.sub` e          -- (1 − φ)
      ep = powPU e'                    -- (1 − φ)^p
      w  = g `Torch.mul` ep            -- gᵢ · (1−φᵢ)^p
      sW = Torch.sumAll w
      sG = Torch.sumAll g
      m  = sW `Torch.div` (sG `Torch.add` epsU)
   in UnsafeMkTensor (Torch.reshape [1] (oneU `Torch.sub` powInvPU m))
...
bigOplusU g phi =
  let w  = g `Torch.mul` phi
      sW = Torch.sumAll w
      sG = Torch.sumAll g
   in UnsafeMkTensor (Torch.reshape [1] (sW `Torch.div` (sG `Torch.add` epsU)))
...
pi0U x = (x `Torch.mul` stableScaleU) `Torch.add` stableEpsU

-- | π₁: maps [0,1] → [0, 1)
pi1U :: Torch.Tensor -> Torch.Tensor
pi1U x = x `Torch.mul` stableScaleU
