...
import A2_Interpretation.B4_NonLogical.MNIST_MLP (MLP (..), currentModel, hTheta, mnistSpec)
...
instance MNIST_Vocab DATA where
  ...
  digit x = Dist [(mnistLabels !! fromIntegral x, 1.0)]

  add :: (Image DATA, Image DATA) -> Digit DATA
  add (x, y) = mnistMapDATA Map.! (x, y)
...
instance MNIST_Vocab TENS where
  ...
  digit imgTensor = Identity $ hTheta currentModel (toDynamic imgTensor)
...
instance MNIST_Bridge DATA TENS where
  ...
  decDigit logits =
    let probs = Torch.asValue (F.softmax (Torch.Dim 0) logits) :: [Float]
     in Dist [(fromIntegral i, realToFrac p) | (i, p) <- zip [0 :: Int .. 9] probs]
