...
-- | Inference inhabitation: concrete implementations of all vocabulary symbols.
module E_Inference.C_Inhabitation.InferenceInhabTens () where

import E_Inference.B_Vocabulary.InferenceVocabTens (InferenceVocab (..))
import qualified Torch

-- | Inhabitation of InferenceVocab for Torch.Tensor.
instance InferenceVocab Torch.Tensor where
  -- \| softplus(sat) = -log(σ(sat))
  ...
  -- \| convex(J_data, J_know, λ) = λ · J_data + (1-λ) · J_know
