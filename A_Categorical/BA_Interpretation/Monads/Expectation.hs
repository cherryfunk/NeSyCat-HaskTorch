...
module A_Categorical.BA_Interpretation.Monads.Expectation (HasExpectation (..), probGiry, probDist) where
...
instance HasExpectation Giry where
  ...
  expect obj (GBind m k) f = evalBind m (\x -> expect obj (k x) f)
  -- 1. Reals (R) -- Categorical on Reals is an error. Use continuous constructors.
  expect Reals (GFiniteSupp _) _ = error "Giry: Categorical on Reals -- use Normal, Uniform, GenericCont, etc."
  expect Reals giry f = evalContinuous giry f
  -- 2. Integers (Z) -- countably infinite: chained convergence strategy.
  expect Integers (GFiniteSupp xs) f = chainedDiscreteStrategy xs f
  -- 3. Strings (countably infinite, like Integers).
  expect Strings (GFiniteSupp xs) f = chainedDiscreteStrategy xs f
  -- 4. Booleans ({True, False}) -- finite set with exactly 2 elements.
  expect Booleans (GFiniteSupp xs) f = sum [p * f x | (x, p) <- xs]
  -- 5. Finite sets ({x_1, ..., x_n}) -- finite direct summation.
  expect (Finite _) (GFiniteSupp xs) f = sum [p * f x | (x, p) <- xs]
  ...
  expect (Prod _ _) (GFiniteSupp xs) f = sum [p * f x | (x, p) <- xs]
  -- DisUniform: desugar to Categorical with uniform weights
  expect obj (GFinUniform xs) f = expect obj (GFiniteSupp [(x, 1.0 / fromIntegral (length xs)) | x <- xs]) f
...
instance HasExpectation Dist where
  ...
  expect _ d f = evalDist d f

-- | Internal evaluator for Dist (handles existential in Bind).
evalDist :: Dist a -> (a -> Double) -> Double
evalDist (Pure x) f = f x
evalDist (Bind m k) f = evalDist m (\x -> evalDist (k x) f)
evalDist (FiniteSupp xs) f = sum [p * f x | (x, p) <- xs]
evalDist (FinUniform xs) f = evalDist (FiniteSupp [(x, 1.0 / fromIntegral (length xs)) | x <- xs]) f
