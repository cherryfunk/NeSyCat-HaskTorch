...
RTS_FLAGS="+RTS -A64m -RTS"

mkdir -p "$RESULTS"

echo "=== Building all binary executables ==="
cd "$REPO"
cabal build binary-test-real binary-test-jit-real binary-test-real-beta 2>&1
...
run_haskell() {
  ...
  rm -f "$RESULTS/${name}_all.txt"
  for i in $(seq 1 $N); do
    echo "--- Run $i ---"
    "$bin" $RTS_FLAGS 2>&1 | tee -a "$RESULTS/${name}_all.txt"
  done
}
