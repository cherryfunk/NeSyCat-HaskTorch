...
--
--   Prediction = Double (expectation of Dist Bool from classifierA @DATA)
--   Label      = Bool   (from labelA @DATA)
module F_Benchmark.F_Interpretation.BenchmarkIntpData
  ()
where

import F_Benchmark.B_Vocabulary.BenchmarkVocab (BenchmarkVocab (..))
import F_Benchmark.C_Inhabitation.BenchmarkInhabData ()
import F_Benchmark.D_Theory.BenchmarkTheory (BenchmarkFun (..))
