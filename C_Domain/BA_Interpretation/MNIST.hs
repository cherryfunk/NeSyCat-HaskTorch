...
module C_Domain.BA_Interpretation.MNIST
  ( mnistTable,
    mnistMapDATA,
    mnistTableTENS,
    setGlobalMLP,
    module C_Domain.B_Theory.MnistTheory,
    module C_Domain.BA_Interpretation.MNIST_MLP,
  )
where
...
instance MnistBridge DATA TENS where
  ...
  decDigit logits =
    ...
    let maxIdx = Torch.asValue (Torch.argmax (Torch.Dim 0) Torch.RemoveDim logits) :: Int
     in FiniteSupp [(fromIntegral maxIdx, 1.0)]
