...
module B_Interpretation.C_Logical.Real where

import A_Syntax.C_Logical.A2MonBLat (A2MonBLat (..))
import A_Syntax.C_Logical.TwoMonBLat (TwoMonBLat (..))
import C_Semantics.A_Categorical.Monads.Giry (Giry (..))
import B_Interpretation.B_Typological.DATA (DATA (..))
import B_Interpretation.D_NonLogical.Supremum (enumAll, inf, sup)
