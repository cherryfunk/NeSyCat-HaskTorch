...
module B_Interpretation.B_Logical.Real where

import A_Syntax.B_Logical.A2MonBLat (A2MonBLat (..))
import A_Syntax.B_Logical.TwoMonBLat (TwoMonBLat (..))
import C_Semantics.A_Categorical.Monads.Giry (Giry (..))
import B_Interpretation.B_Typological.DATA (DATA (..))
import B_Interpretation.C_NonLogical.Supremum (enumAll, inf, sup)
import C_Semantics.A_Categorical.Monads.Expectation (HasExpectation (..))
...
instance A2MonBLat DATA Omega where
