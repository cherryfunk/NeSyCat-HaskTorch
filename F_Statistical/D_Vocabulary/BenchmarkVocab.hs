...
module F_Statistical.D_Vocabulary.BenchmarkVocab
  ( BenchmarkVocab (..),
  )
where
...
class BenchmarkVocab cat where
  -- | threshold : cat x cat -> Bool.  Predicted probability > threshold.
  threshold :: cat -> cat -> Bool

  -- | countTrue : [Bool] -> cat.  Fraction of True values (count / total).
  fractionTrue :: [Bool] -> cat

  -- | harmonicMean : cat x cat -> cat.  2ab/(a+b).
  harmonicMean :: cat -> cat -> cat

  -- | meanWhere : [cat] x [Bool] -> cat.  Mean of values where mask is True.
