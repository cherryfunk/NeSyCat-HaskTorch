...
-- | JIT Training loop for Binary Classification using TensUniform logic.
module D_Inference.C_NonLogical.BinaryTrainingUniformJIT
  ( trainBinaryUniformJIT,
  )
where

import B_Interpretation.C_NonLogical.BinaryUniformMLP (Binary_MLP, binarySpec)
import B_Interpretation.C_NonLogical.BinaryUniform (setGlobalBinaryMLP)
import qualified B_Interpretation.B_Logical.TensUniform as TENS
...
-- | JIT Training loop (TensUniform).
--   The axiom takes training data (empirical measure) and the model.
trainBinaryUniformJIT :: Int -> Float -> (Torch.Tensor -> Binary_MLP -> TENS.Omega) -> IO (Binary_MLP, Torch.Tensor, Torch.Tensor, Torch.Tensor, Torch.Tensor)
trainBinaryUniformJIT numEpochs learningRate kbSatFormula = do
  ...
  rawMod <- trace "axiom" "forward"
    (\[d, pp] -> do
       let paramTensors = unpackParams paramShapes pp
           ...
           -- Uniform loss = 1.0 - sat
           loss = oneTens `Torch.sub` satDyn
       return [loss]
    )
